...
from code.helper import prepare_symptoms_array, get_disease_description, get_disease_precautions
...
@st.cache_data
def process_prediction(_model, data):
...
print(f"UI co ban da san sang: {ui_duration:.6f} giây")
...
def display_prediction_result(prediction, disease_name, name=""):
    if prediction == 1:
        image = load_image('positive.jpg')
        message = f"Chúng tôi rất tiếc phải thông báo rằng bạn có khả năng mắc {disease_name}."
    else:
        image = load_image('negative.jpg')
        message = f"Chúc mừng, bạn không mắc {disease_name}."
...
# Diabetes prediction page
elif selected == 'Dự Đoán Tiểu Đường':
    ...
    if st.button("Kết quả kiểm tra tiểu đường"):
        diabetes_prediction=[[]]
        diabetes_model = get_model('diabetes', "models/diabetes_model.sav")
...
if selected == 'Dự Đoán Bệnh Tim':
    ...
    if st.button("Kết quả kiểm tra tim"):
        heart_model = get_model('heart', "models/heart_disease_model.sav")
...
#    name	MDVP:Fo(Hz)	MDVP:Fhi(Hz)	MDVP:Flo(Hz)	MDVP:Jitter(%)	MDVP:Jitter(Abs)	MDVP:RAP	MDVP:PPQ	Jitter:DDP	MDVP:Shimmer	MDVP:Shimmer(dB)	Shimmer:APQ3	Shimmer:APQ5	MDVP:APQ	Shimmer:DDA	NHR	HNR	status	RPDE	DFA	spread1	spread2	D2	PPE
   # change the variables according to the dataset used in the model
    ...
    if st.button("Kết quả kiểm tra Parkinson"):
        parkinson_model = get_model('parkinson', "models/parkinsons_model.sav")
...
if selected == 'Dự Đoán Ung Thư Phổi':
    ...
    if st.button("Dự đoán ung thư phổi"):
        ...
        # Perform prediction
        lung_cancer_model = get_model('lung_cancer', 'models/lung_cancer_model.sav')
...
# st.write(info.astype(int).info())
    ...
    if st.button("Kết quả kiểm tra gan"):
        liver_model = get_model('liver', 'models/liver_model.sav')
...
if selected == 'Dự Đoán Viêm Gan':
    ...
    if st.button("Dự đoán viêm gan"):
        ...
        # Perform prediction
        hepatitis_model = get_model('hepatitis', 'models/hepititisc_model.sav')
...
if selected == 'Dự Đoán Bệnh Thận Mãn Tính':
    ...
    if st.button("Dự đoán bệnh thận mãn tính"):
        ...
        # Perform prediction
        chronic_disease_model = get_model('chronic', 'models/chronic_model.sav')
...
if selected == 'Dự Đoán Ung Thư Vú':
    ...
    if st.button("Dự đoán ung thư vú"):
        ...
        # Perform prediction
        breast_cancer_model = get_model('breast_cancer', 'models/breast_cancer.sav')
